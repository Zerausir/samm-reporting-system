...
class SammApp(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.button1 = tk.Checkbutton(self.master, text="Mapa.", variable=self.mapa,
    ...
    def program(self):
        ...
        if self.repgen.get():
            ...
            df.to_csv(f'SessionSummary_{czo}.csv', index=False, sep=';',
                      encoding='utf-8',
                      header=True, decimal=',')

            # Remove the previous files if already exist
            if os.path.exists(
                    f"{os.getenv('download_route')}/SessionSummary_{czo}.csv"):
                os.remove(
                    f"{os.getenv('download_route')}/SessionSummary_{czo}.csv")

            # Download the .csv file
            os.rename(f'SessionSummary_{czo}.csv',
                      f"{os.getenv('download_route')}/SessionSummary_{czo}.csv")
            ...
            datos1 = {'Prestador': ['Conecel', 'CNT EP', 'Otecel'],
                      '% Sesiones HTTP Fallidas 3G': [df_claro_3g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_claro_3g['EndServiceStatus'].value_counts()['Succeeded'] * 100,
                                                      df_cnt_3g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_cnt_3g['EndServiceStatus'].value_counts()['Succeeded'] * 100,
                                                      df_movistar_3g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_movistar_3g['EndServiceStatus'].value_counts()[
                                                          'Succeeded'] * 100],
                      '% Sesiones HTTP Fallidas 4G': [df_claro_4g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_claro_4g['EndServiceStatus'].value_counts()['Succeeded'] * 100,
                                                      df_cnt_4g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_cnt_4g['EndServiceStatus'].value_counts()['Succeeded'] * 100,
                                                      df_movistar_4g['EndServiceStatus'].value_counts()['Failed'] /
                                                      df_movistar_4g['EndServiceStatus'].value_counts()[
                                                          'Succeeded'] * 100],
            ...
            with pd.ExcelWriter(f'{download_route}/Reporte.xlsx') as writer:
            ...
            """Rename the file"""
            os.rename(f'{download_route}/{old_name}', f'{download_route}/{new_name}')
