    ...
    def program(self):
        ...
        if self.repgen.get():
            ...
            def get_dataframes(df):
                ...
                session_types = {'ul_http': 'HTTP Post', 'dl_http': 'HTTP Download'}
                df_dict = {}

                for op in operators:
                    for st in session_types:
                        df_name = f"df_{op.lower()}_{st}"
                        df_filtered = df.loc[(df['SimOperator'] == op) &
                                             (df['SessionType'] == session_types[st]) &
                                             (df['EndServiceStatus'].isin(['Failed', 'Succeeded']))]
                        df_dict[df_name] = df_filtered

                        df_type_name = f"{df_name.lower()}"
                        df_dict[f"{df_type_name}_fail"] = df_dict[df_name].loc[
                            df_dict[df_name]['EndServiceStatus'] == 'Failed']
                        df_dict[f"{df_type_name}_succ"] = df_dict[df_name].loc[
                            df_dict[df_name]['EndServiceStatus'] == 'Succeeded']
            ...
            datos1 = {'Prestador': ['Conecel', 'CNT EP', 'Otecel'],
                      '% Sesiones HTTP Fallidas': [
                          divide_or_nan((df_dict['df_claro_ul_http_fail']['EndServiceStatus'].count() +
                                         df_dict['df_claro_dl_http_fail']['EndServiceStatus'].count()), (
                                                df_dict['df_claro_ul_http_succ']['EndServiceStatus'].count() +
                                                df_dict['df_claro_dl_http_succ']['EndServiceStatus'].count())) * 100,
                          divide_or_nan((df_dict['df_cnt_ul_http_fail']['EndServiceStatus'].count() +
                                         df_dict['df_cnt_dl_http_fail']['EndServiceStatus'].count()), (
                                                df_dict['df_cnt_ul_http_succ']['EndServiceStatus'].count() +
                                                df_dict['df_cnt_dl_http_succ']['EndServiceStatus'].count())) * 100,
                          divide_or_nan((df_dict['df_movistar_ul_http_fail']['EndServiceStatus'].count() +
                                         df_dict['df_movistar_dl_http_fail']['EndServiceStatus'].count()), (
                                                df_dict['df_movistar_ul_http_succ']['EndServiceStatus'].count() +
                                                df_dict['df_movistar_dl_http_succ'][
                                                    'EndServiceStatus'].count())) * 100],
                      'Promedio HTTP UL Mbps': [df_dict['df_claro_ul_http']['ThroughputMbps'].mean(),
                                                df_dict['df_cnt_ul_http']['ThroughputMbps'].mean(),
                                                df_dict['df_movistar_ul_http']['ThroughputMbps'].mean()],
                      'Promedio HTTP DL Mbps': [df_dict['df_claro_dl_http']['ThroughputMbps'].mean(),
                                                df_dict['df_cnt_dl_http']['ThroughputMbps'].mean(),
                                                df_dict['df_movistar_dl_http']['ThroughputMbps'].mean()],
                      'Total Sesiones HTTP UL': [df_dict['df_claro_ul_http']['EndServiceStatus'].count(),
                                                 df_dict['df_cnt_ul_http']['EndServiceStatus'].count(),
                                                 df_dict['df_movistar_ul_http']['EndServiceStatus'].count()],
                      'Total Sesiones HTTP DL': [df_dict['df_claro_dl_http']['EndServiceStatus'].count(),
                                                 df_dict['df_cnt_dl_http']['EndServiceStatus'].count(),
                                                 df_dict['df_movistar_dl_http']['EndServiceStatus'].count()],
                      'Total tareas HTTP UL': [df_dict['df_claro_ul_http']['ThroughputMbps'].count(),
                                               df_dict['df_cnt_ul_http']['ThroughputMbps'].count(),
                                               df_dict['df_movistar_ul_http']['ThroughputMbps'].count()],
                      'Total tareas HTTP DL': [df_dict['df_claro_dl_http']['ThroughputMbps'].count(),
                                               df_dict['df_cnt_dl_http']['ThroughputMbps'].count(),
                                               df_dict['df_movistar_dl_http']['ThroughputMbps'].count()]}
            df_1 = pd.DataFrame(data=datos1)
            df_1['% Sesiones HTTP Fallidas'] = df_1['% Sesiones HTTP Fallidas'].round(2)
            df_1['Promedio HTTP UL Mbps'] = df_1['Promedio HTTP UL Mbps'].round(2)
            df_1['Promedio HTTP DL Mbps'] = df_1['Promedio HTTP DL Mbps'].round(2)
            df_1 = df_1.fillna('-')

            """REPORT CREATION"""
            """create, write and save"""
            with pd.ExcelWriter(f"{os.getenv('download_route')}/Reporte.xlsx", engine='xlsxwriter') as writer:
